// ═══════════════════════════════════════════════════════════════
// MODULE: Hit Score Checklist  |  hitscore/index.js
// Pipeline OS · v4.1 · src/modules/hitscore/index.js
...
const def = {
  id: 'hitscore',
  icon: '📊',
  title: 'Hit Score Checklist',
  subtitle: '66 criteria · Spotify 1M+ research',
  color: 'var(--purple)',
  contract: {
    needs: ['lyrics.sections', 'meta'],
    delivers: [],
  },
};
...
export const HS_TOTAL = HS_CATEGORIES.flatMap(c => c.items).length;
export const HS_CRITICAL = HS_CATEGORIES.flatMap(c => c.items).filter(i => i.critical).length;
...
function bodyHitScore(idx) {
  ...
  const obj = d.obj || (typeof window.pipelineObj !== 'undefined' ? window.pipelineObj : null);
  ...
  // ── Analyze button ──
  const apiKey = (typeof window.getKey === 'function' ? window.getKey() : '');
  const hsRunning = d.hsRunning || false;
  h += `<div style="padding:10px 16px;display:flex;gap:8px;align-items:center;border-bottom:1px solid var(--border);flex-wrap:wrap;">`;
  h += `<button class="m-btn m-btn-ai" style="font-size:10px;padding:5px 14px;" onclick="hsAnalyze(${idx})" ${!hasLyrics || !apiKey || hsRunning ? 'disabled' : ''}>`;
  h += hsRunning ? '⏳ Analyzing...' : '🤖 Analyze with AI';
  h += `</button>`;
  if (!apiKey) h += `<span style="font-size:9px;color:var(--muted)">Enter API key at top to analyze</span>`;
  else if (!hasLyrics) h += `<span style="font-size:9px;color:var(--muted)">Needs lyrics to analyze</span>`;
  if (d.hsAiNote) h += `<span style="font-size:9px;color:var(--green);margin-left:auto">${esc(d.hsAiNote)}</span>`;
  h += `</div>`;
...
// ── AI Analysis ──────────────────────────────────────────────
async function hsAnalyze(idx) {
  const p = window.pipeline[idx]; if (!p) return; if (!p.data) p.data = {};
  const d = p.data;
  const apiKey = typeof window.getKey === 'function' ? window.getKey() : '';
  if (!apiKey) { window.showToast('Enter API key at top first'); return; }

  const obj = d.obj || window.pipelineObj;
  const sections = obj?.lyrics?.sections || [];
  if (!sections.some(s => s.content?.trim())) { window.showToast('No lyrics to analyze'); return; }

  d.hsRunning = true;
  window.renderModules();

  // Build lyrics text with section names and prompts
  let lyricsText = '';
  const secPrompts = obj?.prompts_seccionais || {};
  sections.forEach(s => {
    if (s.name) {
      const sp = secPrompts[s.name];
      const prompt = sp?.positive && sp.positive !== '__wizard__' ? sp.positive : '';
      lyricsText += `[${s.name}${prompt ? ': ' + prompt : ''}]\n`;
    }
    if (s.content) lyricsText += s.content + '\n\n';
  });

  const meta = obj?.meta || {};
  let metaText = '';
  if (meta.bpm) metaText += `BPM: ${meta.bpm}\n`;
  if (meta.key) metaText += `Key: ${meta.key}\n`;
  if (meta.timeSig) metaText += `Time Signature: ${meta.timeSig}\n`;
  if (meta.style) metaText += `Style: ${meta.style}\n`;
  if (obj?.prompt_overall?.positive) metaText += `Overall Prompt: ${obj.prompt_overall.positive}\n`;

  // Build criteria list for the prompt
  const allItems = HS_CATEGORIES.flatMap(c => c.items.map(i => ({
    id: i.id,
    category: c.title,
    text: i.text,
    critical: i.critical,
  })));
  const criteriaJSON = JSON.stringify(allItems);

  const systemPrompt = `You are a hit song analyst. You will analyze lyrics and metadata against a checklist of 66 criteria used by top Spotify producers. For each criterion, determine if the song meets it based on the available information. Be strict but fair. If you cannot determine a criterion from lyrics alone (e.g., audio production quality), mark it as unmet. Return ONLY a JSON object with this exact format: {"checked":["d1","s2",...],"summary":"One sentence overall assessment"}. The "checked" array should contain ONLY the IDs of criteria that are clearly met. No markdown, no explanation outside the JSON.`;

  const userMsg = `Analyze this song:\n\n${metaText ? '--- METADATA ---\n' + metaText + '\n' : ''}--- LYRICS ---\n${lyricsText}\n--- CRITERIA ---\n${criteriaJSON}`;

  try {
    const resp = await window.claudeCall(apiKey, {
      max_tokens: 2048,
      system: systemPrompt,
      messages: [{ role: 'user', content: userMsg }],
    });
    const text = window.extractText(resp);
    // Parse JSON from response (handle possible markdown wrapping)
    const jsonStr = text.replace(/```json?\n?/g, '').replace(/```/g, '').trim();
    const result = JSON.parse(jsonStr);

    if (result.checked && Array.isArray(result.checked)) {
      d.hsChecked = {};
      result.checked.forEach(id => { d.hsChecked[id] = true; });
      // Open all categories so user can see results
      d.hsCatOpen = {};
      HS_CATEGORIES.forEach(c => { d.hsCatOpen[c.id] = true; });
      d.hsAiNote = `AI: ${result.checked.length}/${HS_TOTAL} criteria met` + (result.summary ? ' · ' + result.summary : '');
      window.showToast(`Hit Score: ${Math.round((result.checked.length / HS_TOTAL) * 100)}% — ${result.checked.length} criteria met`);
    }
  } catch (e) {
    window.showToast('Analysis failed: ' + e.message);
    console.error('hsAnalyze error:', e);
  } finally {
    d.hsRunning = false;
    window.renderModules();
    window.triggerAutosave('lyric');
  }
}

// ── Self-registration ─────────────────────────────────────────
registerModule(def, bodyHitScore);

// Expose to window
window.HS_CATEGORIES = HS_CATEGORIES;
window.HS_TOTAL = HS_TOTAL;
window.HS_CRITICAL = HS_CRITICAL;
window.hsToggle = hsToggle;
window.hsCatToggle = hsCatToggle;
window.hsTogLyrics = hsTogLyrics;
window.hsFilter = hsFilter;
window.hsReset = hsReset;
window.hsAnalyze = hsAnalyze;

// ═══════════════════════════════════════════════════════════════
// MODULE: Text to Prompter (beta)  |  text2prompter.js
// Pipeline OS · module file · loads via build.sh concatenation
// AI-assisted keyword picker — reference songs → Suno keywords
// Uses Claude API (Anthropic) for song analysis
// ═══════════════════════════════════════════════════════════════

const T2P_MAX_REFS = 3;

// ── Conflict pairs: tags that may cancel each other ──
const T2P_CONFLICTS = [
...
function t2pSaveKey(idx) {
  const inp = getEl('t2p-apikey-' + idx);
  const key = inp?.value?.trim();
  if (!key || !key.startsWith('sk-ant-')) { showToast('Invalid API key format'); return; }
...
function t2pRefUpd(idx, refIdx, val) {
  const p = pipeline[idx]; if (!p?.data) return;
...
async function t2pAnalyze(idx) {
  ...
  if (!refs.length) { showToast('Add at least one reference'); return; }
  const apiKey = localStorage.getItem('ppos_claude_api_key');
  if (!apiKey) { showToast('API key not set'); return; }
  ...
  try {
    const resp = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 1024,
        system: t2pBuildSystemPrompt(),
        messages: [{ role: 'user', content: t2pBuildUserPrompt(refs) }]
      })
    });
...
function t2pApply(idx) {
  const p = pipeline[idx]; if (!p?.data?.t2pResult) return;
  const res = p.data.t2pResult;

  // Find sunobuilder module in pipeline
  const sbIdx = pipeline.findIndex(m => m.defId === 'sunobuilder');
  if (sbIdx < 0) { showToast('Add Suno Prompt Builder to pipeline first'); return; }
  const sb = pipeline[sbIdx];
  if (!sb.data) sb.data = {};
  if (!sb.data.sbVals) {
    sb.data.sbVals = {};
  ...
  // Map result to sbVals
  Object.entries(res).forEach(([key, val]) => {
    if (!val) return;
    if (key === 'bpm') { V.bpm = String(val).replace(/[^\d]/g, ''); return; }
    if (key === 'time_sig') { V.time_sig = String(val); return; }
    if (Array.isArray(val) && val.length) {
      // For chip-based sections, set the values directly
      if (V[key] !== undefined) V[key] = val.map(v => String(v));
    }
  });

  sb.data._synced = true; // prevent pipelineObj overwrite
  p.data.t2pApplied = true;
  renderModules();
  showToast('Applied to Suno Prompter ✓');

  // Scroll to sunobuilder
  const el = getEl('module-' + sbIdx);
  if (el) { pipeline[sbIdx].collapsed = false; el.classList.remove('collapsed'); el.scrollIntoView({ behavior: 'smooth', block: 'start' }); }
}
...
function t2pReset(idx) {
  const p = pipeline[idx]; if (!p?.data) return;
  p.data.t2pResult = null;
  p.data.t2pConflicts = [];
  p.data.t2pError = '';
  p.data.t2pApplied = false;
  renderModules();
}

// ── Self-registration ────────────────────────────────────────
_mr['text2prompter'] = bodyText2Prompter;
MODULE_DEFS.push({ id: 'text2prompter', icon: '🎯', title: 'Text to Prompter', subtitle: 'AI reference analysis · beta', color: 'var(--cyan)' });
